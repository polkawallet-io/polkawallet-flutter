...
async function fetchDexPoolInfo(currencyId, address) {
  ...
  const reward =
    balanceToNumber(pool[3]) * proportion - balanceToNumber(pool[4]);
  return {
    token: currencyId,
    pool: pool[0],
    sharesTotal: pool[1],
...
async function _getStakingPoolHelper(api) {
...
async function fetchLDOTPrice(api) {
  const stakingPoolHelper = await _getStakingPoolHelper(api);
