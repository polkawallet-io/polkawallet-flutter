...
let swapTrader;
function _getSwapTradeInstance(api) {
  if (swapTrader) return;

  const aca = getPresetToken("ACA").clone({
    amount: new FixedPointNumber(100),
  });
...
async function calcTokenSwapAmount(api, input, output, swapPair, slippage) {
  if (!swapTrader) {
    swapTrader = _getSwapTradeInstance(api);
  }
  swapTrader.setMode(output === null ? "EXACT_INPUT" : "EXACT_OUTPUT");
  swapTrader.setInput(
    getPresetToken(swapPair[0]).clone({
      amount: new FixedPointNumber(input || 0),
    })
  );
  swapTrader.setOutput(
    getPresetToken(swapPair[1]).clone({
      amount: new FixedPointNumber(output || 0),
    })
  );
  swapTrader.setAcceptSlippage(new FixedPointNumber(slippage));
  const paths = swapTrader.getTradeTokenPairsByPaths();
  const res = await api.query.queryMulti(
    paths.map((e) => [api.query.dex.liquidityPool, ...e.toChainData()])
  );
  const pools = SwapTrade.convertLiquidityPoolsToTokenPairs(res, paths);
  return swapTrade.getTradeParameters(pools);
}
...
async function fetchDexPoolInfo(currencyId, address) {
  ...
  const reward = balanceToNumber(pool[3][0]) * proportion - balanceToNumber(pool[4]);
  return {
    token: currencyId,
    pool: pool[0],
    sharesTotal: pool[1],
