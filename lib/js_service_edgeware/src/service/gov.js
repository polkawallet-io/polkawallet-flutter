...
import { formatBalance, stringToU8a, BN_ZERO, hexToString, hexToU8a } from "@polkadot/util";
...
async function getReferendumVoteConvictions() {
  const enact =
    ((((PERIODS[api.genesisHash.toHex()] || api.consts.democracy.enactmentPeriod).toNumber() *
...
async function makeTreasuryProposalSubmission(id, isReject) {
  const members = await (api.query.electionsPhragmen || api.query.elections).members();
...
async function getDemocracyUnlocks(address) {
