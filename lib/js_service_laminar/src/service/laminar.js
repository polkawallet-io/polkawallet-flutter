...
const msgChannelPrices = "LaminarPrices";
async function subscribePrices() {
  return api.rpc.oracle
    .getAllValues((res) => {
      const data = {};
      res.forEach((e) => {
        data[e[0].toString()] = e[1].toJSON();
      });
      send(msgChannelPrices, data);
    })
    .then((unsub) => {
      const unsubFuncName = `unsub${msgChannelPrices}`;
